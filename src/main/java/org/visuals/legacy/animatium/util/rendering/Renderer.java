...
public class Renderer implements AutoCloseable {
	...
	private final Map<GpuTextureView, GpuSampler> samplers;
	...
	public void setTexture(int id, GpuTextureView textureView, GpuSampler sampler) {
		...
		this.samplers.put(textureView, sampler);
	}

	public void setTexture(int id, Identifier Identifier, GpuSampler sampler) {
		this.setTexture(id, Minecraft.getInstance().getTextureManager().getTexture(Identifier).getTextureView(), sampler);
	}
		...
		if (!this.setup) {
			...
			try (RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> this.displayName, colorTextureView, OptionalInt.empty(), depthTextureView, OptionalDouble.empty())) {
				...
				for (Map.Entry<String, GpuTextureView> entry : this.textures.entrySet()) {
					renderPass.bindTexture(entry.getKey(), entry.getValue(), samplers.get(entry.getValue()));
				}
